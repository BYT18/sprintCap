...
# Upload Video
# video_path = 'adam.mov'
video_path = './working_vids/david.mov'
...
for i in range(len(ankR_pos)):
    a = compute_angle(kneeR_pos[i], ankR_pos[i], toeR_pos[i])
    A_R_ank.append(a)
...
for key in smoothed_data:
    smoothed_data[key] = smoothed_data[key] + [np.nan] * (max_length - len(smoothed_data[key]))
...
seg_max = {col: [] for col in df.columns}
seg_min = {col: [] for col  in df.columns if not col.startswith("AV")}
roms = {col: [] for col in df.columns if not col.startswith("AV")}
    ...
    for i in range(len(indices)):
        ...
        if i == 0:
            ...
            if col.startswith("AV"):
                lag = get_lag(df[col][indices[i-1]:indices[i]])
                lag = lag / (indices[i]-indices[i-1])
                lag = np.mean(lag)
                perc_cycle_time[col].append(lag)
                print("lag2")
                print(lag)
...
overall_mins = {f'min+SE_{col}': np.mean(means) + se[col] for col, means in seg_min.items()}
overall_maxs = {f'max+SE_{col}': np.mean(means) + se[col] for col, means in seg_max.items()}
overall_roms = {f'rom+SE_{col}': np.mean(means) + se[col] for col, means in roms.items()}
overall_perc_cycles = {f'%_cycle_{col}': np.mean(means)*100 for col, means in perc_cycle_time.items()}
...
combined_means = {'Video':video_path, 'PB':10.77, **overall_mins, **overall_maxs, **overall_roms,**overall_perc_cycles, **overall_cons}
